...
import useGlobalContext, {
  imageUploadingModalType,
} from "@/app/c/GlobalContext";
import $$ from "@/utils/global-variables";
import { fetchImage } from "@/utils/api-handler/fetch";
import { Toast } from "@/utils/toastify";
import Console from "@/utils/console";

const cancelTokenSource: { [key: string]: any } = {};

export default function ImageUploadingModal() {
  const { setImageUploadingModal } = useGlobalContext();
  return useMemo(
    () => (
      <_ImageUploadingModal setImageUploadingModal={setImageUploadingModal} />
    ),
    [setImageUploadingModal]
  );
}

function _ImageUploadingModal({
  setImageUploadingModal,
}: {
  setImageUploadingModal: Dispatch<SetStateAction<imageUploadingModalType>>;
}) {
  Console.component("ImageUploadingModal");

  const closeModal = useCallback(() => {
    closingTransition(() => {
      setImageUploadingModal((prev) => ({
        ...prev,
        isOpen: false,
      }));
    });
  ...
  useEffect(() => {
    if (files) {
      ...
      if (totalFileCount > 1) {
        Toast("Multiple file not allowed");
        setFiles(null);
        return;
      }
      ...
      for (let i = 0; i < totalFileCount; i++) {
        ...
        if (!flag) {
          (async () => {
            ...
            if (fileType !== "Image") {
              Toast(`${fileName} is not image file`);
              setFiles(null);
              return;
            }
            ...
            if (!fileType) return;
              ...
              try {
                ...
                $$.addEditorImageFolderId.push(folderId);
                const imageUrl = `$fileUploadDir/${folderId}/${fileName}`;

                fetchImage(
                  imageUrl.replaceAll("$fileUploadDir", $$.fileUploadDir)
                )
                  .then(() => {
                    const newContent = `<img src="${imageUrl}">`;
                    $$.setImageOnEditor(newContent);

                    setAttachedFiles({});
                    closeModal();
                  })
                  .catch(() => {
                    alert("Image not found after maximum attempts.");
                  });
              } catch (error) {
                setAttachedFiles({});
    ...
    <ModalContainer>
      ...
      >
        <div className="flex justify-center items-center gap-5">
          ...
          <div className="text-2xl fontGs">Image attachment</div>
        </div>
        {/* ---------- */}
